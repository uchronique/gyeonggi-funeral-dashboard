...
def build_arcs(regions: pd.DataFrame, facilities: pd.DataFrame, top_n=3, alpha=1.5) -> pd.DataFrame:
    ...
    if arc_df["deaths"].notna().any():
        ...
        arc_df["arc_width"] = 1.5 + 8.5 * np.sqrt(arc_df["demand_flow"] / mx)
    else:
        arc_df["arc_width"] = 1.5 + 7.0 * arc_df["share"]
    return arc_df
...
if not map_arcs.empty:
    layers.append(
        pdk.Layer(
            ...
            get_source_color=[59, 130, 246, 180],
            get_target_color=[245, 158, 11, 220],
            get_width="arc_width",
            width_units="pixels",
            get_height=0.7,
            pickable=True,
